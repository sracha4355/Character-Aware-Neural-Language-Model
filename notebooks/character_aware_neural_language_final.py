...
class WSJDataloader(Dataset):
  def __init__(
    ...
    for split in ["dev", "test", "train"]:
      ...
      # get each unique token and character
    ...
    for split in ["dev", "test", "train"]:
      ...
      for token in tokens:
        ...
        for char in token.text:
          encoded_tokens[-1].append(self.char_to_int[char])
...
print('loading up the dataloaders')
training_data = WSJDataloader(split="train")
dev_data = WSJDataloader(split="dev")
test_data = WSJDataloader(split="test")
...
def train_model(
  ...
  for epoch in range(start_epoch, EPOCHS):
    ...
    print(f'Epoch: {epoch + 1}, Loss: {total_loss} PPL this epoch: {new_ppl} Time taken to finish epoch: {time.time() - start_time}')
    #save_checkpoint(epoch, total_loss, model, optim, f'checkpoint_small_model_epoch_{epoch}.pth')
...
print('calculating ppl pre-training')
